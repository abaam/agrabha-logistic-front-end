<template>
  <DashboardLayout navClass="border-b-0" mobileTitle="drivers">
    <section class="hidden px-3 py-6 md:block md:p-6">
      ...
      <div class="hidden gap-y-3 rounded-md bg-white py-6 shadow md:grid">
            ...
            <div class="inline-block min-w-full py-2 align-middle">
              <div class="overflow-hidden">
                <table class="min-w-full divide-y divide-grey-light">
                  ...
                  <tbody class="divide-y divide-grey-light bg-white" v-if="drivers != ''">
                    <tr v-for="driver in drivers" :key="driver.id">
                      <td class="whitespace-nowrap px-6 py-4">
                        <div class="text-sm">{{ driver.phone_number }}</div>
                      </td>
                      <td class="whitespace-nowrap px-6 py-4">
                        <div class="text-sm">{{ driver.pin }}</div>
                      </td>
                      <td class="whitespace-nowrap px-6 py-4 w-48">
                        <div v-if="driver.verified == 1">
                          <div
                      ...
                      >
                        <span
                        class="text-blue-light hover:text-blue cursor-pointer view-profile" @click="viewProfile(driver.id)"
                        >View Profile</span>
                      </td>
                    </tr>
                  </tbody>
        ...
        >
          ...
          <div>
            <pagination :pagination="pagination" @paginate="fetchDrivers" />
          </div>
        </div>
      </div>
    </section>
        ...
        <TabPanels>
          <TabPanel>
            <div v-if="verified != ''" class="my-3 grid gap-y-2">
              <div v-for="driver in verified"
                :key="driver.id" class="grid gap-y-3 rounded-md bg-white p-3 shadow"
              >
                <div class="flex items-center justify-between">
                  <p class="font-semibold">Phone Number: {{ driver.phone_number }}</p>
                  <p class="font-bold">PIN: {{ driver.pin }}</p>
                </div>
                ...
                <div class="flex items-center space-x-1 text-blue">
                  ...
                  >
                    ...
                    <span
                    class="text-sm font-semibold text-blue view-profile" @click="viewProfile(driver.id)"
                    >View Profile</span>
                </div>
              </div>
            </div>
            <div v-else class="my-20 grid h-96 place-items-center gap-1 px-3">
              <div class="px-3 py-6 text-center">
                ...
                <p class="block leading-6 text-grey-dark">
                  You'll see all the drivers here.
                </p>
              </div>
            </div>
          </TabPanel>
          <TabPanel>
            <div v-if="not_verified != ''" class="my-3 grid gap-y-2">
              <div v-for="driver in not_verified"
                :key="driver.driver_id" class="grid gap-y-3 rounded-md bg-white p-3 shadow"
              >
                <div class="flex items-center justify-between">
                  <p class="font-semibold">Phone Number: {{ driver.phone_number }}</p>
                  <p class="font-bold">PIN: {{ driver.pin }}</p>
                ...
                <div class="flex items-center space-x-1 text-blue">
                  ...
                  <router-link :to='{name:"driver Details",params:{id:driver.id}}' class="text-sm font-semibold text-blue"
    ...
    <div class="modal-dialog modal-dialog-centered relative w-auto pointer-events-none">
        ...
        <div class="modal-body relative p-4">
          <div v-if="driver_details" class="grid grid-cols-1 gap-6 lg:grid-cols-12">
              ...
              <div class="col-span-1 lg:col-span-9">
                  ...
                  <h2 class="text-2xl font-bold text-zinc-700">{{ driver_details.first_name +' '+ 
                  driver_details.middle_name +' '+ 
                  driver_details.last_name +' '+ 
                  driver_details.name_extention }}</h2>
                  <p class="mt-2 font-semibold text-zinc-700">{{ driver_details.email }}</p>
                  <p class="mt-4 text-zinc-500">
                  <h3 class="text-l font-bold text-zinc-700" v-if="driver_details.house_number || driver_details.street || driver_details.barangay || driver_details.city || driver_details.province || driver_details.zip_code">
                  Address: </h3>
                  {{ driver_details.house_number +' '+ 
                  driver_details.street +' '+ 
                  driver_details.barangay +' '+ 
                  driver_details.city +' '+ 
                  driver_details.province +' '+ 
                  driver_details.zip_code }}</p>
                  </div>
              </div>
          </div>
          <div v-else class="divide-y divide-gray-100 dark:divide-gray-700">
              <p class="block leading-6 text-grey-dark text-center m-5">
                  You'll see the driver details here.
              </p>
          </div>  
        </div>
        <div
...
export default {
  ...
  methods: {
        ...
        axios
          .get(
            process.env.VUE_APP_API +
              `users/drivers?page=${pageNum}&entries=${this.show_entries}`, {
          ...
          .then((response) => {
            this.pagination = response.data.drivers.pagination;
            this.drivers = response.data.drivers.collection;
          });
      } else {
        axios
          .get(
            process.env.VUE_APP_API +
              `users/drivers/search?q=${this.search}&page=${pageNum}&entries=${this.show_entries}`, {
              withCredentials: true,  
              headers: {
              'Content-Type': 'application/json',
              'Authorization': 'Bearer ' + localStorage.getItem('csrf_token'),
    ...
    searchDrivers: _.debounce(function () {
      if (this.search != "") {
        axios
          .get(
            process.env.VUE_APP_API +
              `users/drivers/search?q=${this.search}&page=drivers&entries=${this.show_entries}`, {
              withCredentials: true,
              headers: {
              'Content-Type': 'application/json',
              'Authorization': 'Bearer ' + localStorage.getItem('csrf_token'),
    ...
    showEntries(event) {
      if (this.search == "") {
        axios
          .get(
            process.env.VUE_APP_API + "users/drivers?entries=" + event.target.value, {
              withCredentials: true,
              headers: {
              'Content-Type': 'application/json',
              'Authorization': 'Bearer ' + localStorage.getItem('csrf_token'),
        ...
        axios
          .get(
            process.env.VUE_APP_API +
              `users/drivers/search?q=${this.search}&entries=${this.show_entries}`, {
              withCredentials: true,
              headers: {
              'Content-Type': 'application/json',
              'Authorization': 'Bearer ' + localStorage.getItem('csrf_token'),
    ...
    fetchVerified() {
      axios.get(process.env.VUE_APP_API + `users/drivers`, {
        withCredentials: true,
        headers: {
        'Content-Type': 'application/json',
        'Authorization': 'Bearer ' + localStorage.getItem('csrf_token'),
    ...
    viewProfile(id) {
      let currentObj = this;
      axios.get(process.env.VUE_APP_API + `users/driver/${id}`, {
        withCredentials: true,  
        headers: {
        'Content-Type': 'application/json',
        'Authorization': 'Bearer ' + localStorage.getItem('csrf_token'),
        ...
        this.driver_details = response.data.driver

        if(response.data.driver.middle_name == undefined){
            this.driver_details.middle_name = ''
        }

        if(response.data.driver.name_extention == undefined){
            this.driver_details.name_extention = ''
        }

        if(response.data.driver.house_number == undefined){
            this.driver_details.house_number = ''
        }

        if(response.data.driver.street == undefined){
            this.driver_details.street = ''
        }
      })
